    ...
    .listen('BetEvent', (data) => {
        if (data.auction) {
            ...
            if (data.user) {
                ...
                user_page.closest('body').find('.bet', auction_page).text(user.auction_bet);
                user_page.closest('body').find('.bonus', auction_page).text(user.auction_bonus);
                user_page.closest('body').find('.buy__now_price', auction_page).text(user.full_price);
                user_page.closest('body').find('.auto__bid_inp', auction_page).val(user.auto_bid);
            }
        }
    });
