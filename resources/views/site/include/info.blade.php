...
@else
    ...
    <div class="info">
        ...
        @if($auction['buy_now'])
            <div class="circl">
                <a @if($auction['my_win'] && $auction['exchange']) class="my___win" @endif data-id="{{$auction['id']}}"
    ...
    @if($auction['status'] === \App\Models\Auction\Auction::STATUS_ACTIVE)
        ...
        <form class="auto__bid" method="POST" action="{{route('auction.auto_bid',$auction['id'])}}">
            ...
            <input class="auto__bid_inp" min="0" type="number" name="count" value="{{$auction['autoBid']}}"
            ...
            @if($auction['my_win'])
                <div class="btn win">
                    <a @if($auction['my_win'] && $auction['exchange']) class="my___win"
                       ...
                       href="{{route('payment.auction.order',['id'=>$auction['id'],'step'=>'1'])}}">Оформить заказ</a>
                </div>
            @endif
        @endif
    @endif
    ...
    @if($auction['buy_now'] && !$auction['my_win'])
        <div
            class="buy__now @if(!($auction['status'] === \App\Models\Auction\Auction::STATUS_ACTIVE) && !($auction['status'] === \App\Models\Auction\Auction::STATUS_FINISHED)) mt-100 @elseif(($auction['status'] === \App\Models\Auction\Auction::STATUS_FINISHED) && !$auction['my_win']) mt-90 @endif ">
        ...
        <p class="info__text"
           @if($auction['status'] === \App\Models\Auction\Auction::STATUS_FINISHED) style="margin: 0" @endif>Цена с
            учетом
            сделанных Вами
            ставок</p>
    @endif
