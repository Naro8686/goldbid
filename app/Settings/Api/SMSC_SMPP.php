...
class SMSC_SMPP
{
    ...
    protected const SMSC_HOST = "smpp.smsc.ru";
    protected const SMSC_PORT = 3700;
    protected const SMSC_LOGIN = "GoldBid";
    protected const SMSC_PASSWORD = "gSmn5Dnn2T";
    ...
    public function __construct($port = 0)
    {
        ...
        if ($ip == self::SMSC_HOST) // dns fail
            $ip = "212.24.33.196"; // fixed ip

        $this->socket = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);

        if (!$this->socket || !socket_connect($this->socket, $ip, $port ? $port : self::SMSC_PORT))
